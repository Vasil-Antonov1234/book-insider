...
import UserProfileBook from "../user-profile-book/UserProfileBook.jsx";
...
export default function UserProfile() {
    ...
    const ratedBooks = books.filter((book) => book.isRated.includes(user._id));
                    ...
                    <div className="relative overflow-hidden rounded-lg border bg-gray-100/50 p-2">
                        <div className="flex flex-col justify-between rounded-md p-6">
                            <div className="space-y-2">
                                ...
                                <ul>
                                    {myBooks.map((book) => <UserProfileBook key={book._id} book={book}/>)}
                                </ul>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="relative overflow-hidden rounded-lg border bg-gray-100/50 p-2">
                        <div className="flex flex-col justify-between rounded-md p-6">
                            <div className="space-y-2">
                                ...
                                {myComments.map((comment) => <UserProfileBook key={comment._id} comment={comment}/>)}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="relative overflow-hidden rounded-lg border bg-gray-100/50 p-2">
                        <div className="flex flex-col justify-between rounded-md p-6">
                            <div className="space-y-2">
                                ...
                                <ul>
                                    {ratedBooks.map((ratedBook) => <UserProfileBook key={ratedBook._id} ratedBook={ratedBook}/>)}
                                </ul>
                            </div>
                        </div>
                    </div>
